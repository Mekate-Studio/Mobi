...
max_attempts="${KOTLIN_MISSING_ARTIFACT_RETRIES:-20}"
log_dir="${project_root}/build/logs"
started_at="$(date +%s)"

export KOTLIN_CLI_BOOTSTRAP_CACHE_DIR="${KOTLIN_CLI_BOOTSTRAP_CACHE_DIR:-${project_root}/.kotlin-cache}"
export KOTLIN_CLI_USER_HOME="${KOTLIN_CLI_USER_HOME:-${project_root}/.kotlin-user-home}"
export KOTLIN_CLI_TMP_DIR="${KOTLIN_CLI_TMP_DIR:-${project_root}/build/tmp/kotlin}"
...
download_maven_relative_path() {
  ...
  echo "Downloading missing Kotlin Toolchain artifact:"
  echo "  ${artifact_url}"
...
cache_path_for_maven_relative_path() {
  ...
  printf '%s\n' "${KOTLIN_CLI_USER_HOME:?}/Library/Caches/JetBrains/Kotlin/.m2.cache/${maven_relative_path}"
}
...
materialize_missing_dependency() {
  ...
  if [[ -n "${sibling_file}" && ! -f "${artifact_dir}/${sibling_file}" ]]; then
    echo "Materializing related Kotlin KLIB artifact:"
...
attempt=1
while true; do
  attempt_log="${log_dir}/kotlin-command-attempt-${attempt}-$$.log"
  rate_limited_urls=""

  echo "Running Kotlin Toolchain attempt ${attempt}/${max_attempts}: ./kotlin $*"

  set +e
  ./kotlin "$@" 2>&1 | tee "${attempt_log}"
  status=${PIPESTATUS[0]}
  set -e
  ...
  if [[ -n "${rate_limited_urls}" && "${attempt}" -lt "${max_attempts}" ]]; then
    echo
    echo "Kotlin Toolchain hit Maven Central rate limiting. Hydrating reported URLs sequentially and retrying..."
  ...
  echo
  echo "Kotlin Toolchain returned a missing dependency path. Materializing it and retrying..."
  echo "  ${missing_path}"
  if ! materialize_missing_dependency "${missing_path}"; then
    echo "Could not materialize missing Kotlin Toolchain dependency path."
    break
  fi
  attempt=$((attempt + 1))
done
